...
Result: Completed audit (`148/150` test cases passed).

## Scope

- This checklist has been refreshed against the current codebase and current console UI flow.
- It covers setup/config, CSV loading, engine rules, CLI behavior, endgame logic, configurable gem-bank counts, and `EASY` / `MEDIUM` / `HARD` AI behavior.
- Detailed checklist status was mapped from the latest grouped automated audit plus targeted regression probes where one executed probe covered multiple adjacent checklist items.
...
- [x] TC001: Start a 2-player game using default config and verify bank setup is 4 of each non-gold gem, 5 gold, 3 nobles in play, 4 visible cards per tier when enough cards exist, and initial visible deck counts are 36 / 26 / 16.
- [x] TC002: Start a 3-player game using default config and verify bank setup is 5 of each non-gold gem, 5 gold, 4 nobles in play, 4 visible cards per tier when enough cards exist, and initial visible deck counts are 36 / 26 / 16.
- [x] TC003: Start a 4-player game using default config and verify bank setup is 7 of each non-gold gem, 5 gold, 5 nobles in play, 4 visible cards per tier when enough cards exist, and initial visible deck counts are 36 / 26 / 16.
- [x] TC004: Verify the first player entered during setup always gets the first turn.
- [x] TC005: Temporarily remove or rename `config.properties` and verify startup still succeeds by falling back to the default card path, noble path, `win.points=15`, and default gem-bank counts.
...
- [x] TC009: Set `win.points` to `0` or a negative value and verify it falls back to 15 instead of using a non-positive target.
- [x] TC010: Set `gembank.standard.count.2p` to a valid non-default value such as `9` and verify a 2-player game uses that count for all non-gold gem colors while gold remains 5.
- [x] TC011: Remove, blank out, or set an invalid/non-positive `gembank.standard.count.2p` value and verify a 2-player game falls back to 4 per non-gold color.
- [x] TC012: Set `gembank.standard.count.3p` to a valid non-default value and verify a 3-player game uses the configured non-gold count.
- [x] TC013: Set `gembank.standard.count.4p` to a valid non-default value and verify a 4-player game uses the configured non-gold count.
- [x] TC014: Remove or blank out `cards.filepath` or `nobles.filepath` in `config.properties` and verify the default bundled CSV files are used.
- [x] TC015: Point `cards.filepath` or `nobles.filepath` to a missing file and verify startup fails with a clear error instead of starting a partial game.
- [x] TC016: Use a valid alternate card and/or noble CSV file and verify the game loads it correctly.
- [x] TC017: Use a malformed card CSV containing an invalid tier number and verify startup fails safely.
- [x] TC018: Use a malformed card CSV containing missing columns or invalid numeric fields and verify startup fails safely.
- [x] TC019: Use a malformed noble CSV containing missing columns or invalid numeric fields and verify startup fails safely.
...
- [x] TC038: During the different-gems flow, enter `0` on the second or third gem pick and verify the entire action is cancelled cleanly.
...
- [x] TC041: Directly exercise the engine path for taking different gems with duplicate colors and verify it is rejected with no state change.
- [x] TC042: Directly exercise the engine path for taking different gems including `GOLD` and verify it is rejected with no state change.
- [x] TC043: Reduce the bank so only 1 or 2 non-gold colors remain available and verify the CLI adapts the action label to `Take 1 different gem` or `Take 2 different gems` instead of disabling the action.
...
- [x] TC048: Verify the second gem-pick menu in the different-gems flow removes the first chosen color from the option list.
- [x] TC049: Verify the third gem-pick menu removes both previously chosen colors from the option list.
- [x] TC050: Put a player at 10 tokens, take 3 different gems, and verify the action succeeds, the player temporarily exceeds 10, and forced discard begins.
- [x] TC051: Put a player at 9 tokens, take 2 of the same gem, and verify the action succeeds, the player temporarily exceeds 10, and forced discard begins.
...
- [x] TC063: Put a player at 10 tokens, reserve a card while gold is available, and verify the action succeeds, the player temporarily exceeds 10, and forced discard begins.
...
- [x] TC120: Give `EASY` no buy actions but both a legal different-gems action and a legal 2-same-gems action and verify it chooses the different-gems action first.
...
- [x] TC126: Give `MEDIUM` a clear target shortage and verify it chooses a legal targeted gem action when a legal different-gems or 2-same option exists.
...
- [ ] TC150: Re-test previously risky areas and newly audited edge cases together: final-round ending, winner display on ties, discard-after-action flow, AI action rendering, duplicate-name setup, configurable gem-bank startup, and AI fallback gem-taking.

## Notes

- Grouped automated audit execution completed against the current build: `42/42` grouped audit cases passed.
- Detailed checklist status above was then mapped from that grouped audit plus targeted regression probes.
- Remaining failing checklist items: `TC020` and `TC150`.
- `TC020` still fails because duplicate player names are keyed through AI setup by raw player name, so human/AI assignment can cross-wire players who share the same name.
- `TC043` was updated to the current implementation: the app now allows taking `1-3` different gems and dynamically relabels the action based on how many colors are available.
- `win.points<=0` now falls back to `15`; it no longer starts a game with a non-positive target.
